...
test('useCase() creates the case function', () => {
...
test('useCase() calls the factory method when executes the case', async () => {
  const createMock = vi.spyOn(TestCase, 'create');
...
test('useCase() calls the execute method', async () => {
  const executeMock = vi.spyOn(TestCase.prototype, 'execute');
...
test('useCase() calls the onAbort method when aborts the case', async () => {
  const onAbortMock = vi.spyOn(TestCase.prototype, 'onAbort');
